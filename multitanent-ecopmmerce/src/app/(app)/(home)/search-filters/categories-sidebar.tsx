  ...
  const handleCategoryClick = (category: CustomCategory) => {
    if (category.subcategories && category.subcategories.length > 0) {
      ...
      if (parentCategories && selectedCategory) {
        //this is sunbcategory navigate to category/subcategory
        ...
        <ScrollArea className="flex flex-col overflow-y-auto  pb-2 h-full">
          ...
          {currentCategories.map((category) => (
            <button
              ...
              key={category.slug}
              className="w-full text-left p-4 cursor-pointer hover:bg-black hover:text-white flex justify-between items-center text-base font-medium"
            >
              {category.name}
